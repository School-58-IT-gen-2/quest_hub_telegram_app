...
async def change_char_age(message: types.Message, state: FSMContext):
    ...
    if age.isdigit():
        ...
        await message.answer(text=f'{await tg_text_convert("Тестировщик заходит в бар и заказывает: кружку пива, 2 кружки пива, 0 кружек пива, 999999999 кружек пива, ящерицу в стакане, -1 кружку пива, qwertyuip кружек пива. Первый реальный клиент заходит в бар и спрашивает, где туалет. Бар вспыхивает пламенем, все погибают.")} Пожалуйста, введите корректный возраст\.',parse_mode="MarkdownV2")
